...
# Chạy dưới quyền user thường, không dùng root
RUN useradd --no-create-home --shell /usr/sbin/nologin appuser
USER appuser

EXPOSE 8000

# Health check — đọc PORT động, gọi /healthz (image slim không có curl)
...
# Shell form để expand biến $PORT khi chạy trên cloud
